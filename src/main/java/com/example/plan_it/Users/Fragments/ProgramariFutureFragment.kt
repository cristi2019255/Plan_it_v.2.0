...
import com.example.plan_it.Users.Adapter.ProgramareFutureAdapter
...
class ProgramariFutureFragment(var UserId:String) : Fragment() {
    private var programariFutureRecyclerView: RecyclerView? = null
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_programari_future, container, false)
        programariFutureRecyclerView = view.findViewById(R.id.programari_fragment_programari_future)
        programariFutureRecyclerView!!.setHasFixedSize(true)
        programariFutureRecyclerView!!.layoutManager = LinearLayoutManager(context)
                ...
                if (snapshot.getValue(t) != null) {
                    ...
                    for (i in 0..(map.size - 1)) {
                        ...
                        if (!cal.before(calCurrent)) {
                            listFuture.add("${map.get(i)} ${hashmap.get(map.get(i))}")
                        }
                    }
                    var programariFutureAdapter = ProgramareFutureAdapter(context!!, listFuture,UserId)
                    programariFutureRecyclerView!!.setAdapter(programariFutureAdapter)
                }
            }
        })
        return view
